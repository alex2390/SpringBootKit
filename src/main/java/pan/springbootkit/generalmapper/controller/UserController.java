...
public class UserController {
	 ...
	 * @author panzhangbao
	 */
	@RequestMapping(value = "{id:\\d+}", method = {RequestMethod.GET, RequestMethod.DELETE})
	public BaseResult getOrDelete(@PathVariable("id") Long id, HttpServletRequest request) {
		User user = userService.getOrDelete(id, request.getMethod());
		if (null == user) {
	...
	public BaseResult insertOrUpdate(@RequestBody User user, HttpServletRequest request) {
		 ...
		 */
		if (null == user.getId()) {
			return BaseResult.illegalParam("id");
		}
